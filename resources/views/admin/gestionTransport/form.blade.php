    ...
    <form action="{{ route('gestion.store', $evenement)}}" class="vstack gap-2" method="post">
        ...
        <fieldset>
            ...
            <div class="row">
                ...
                @include('shared.input', ['name'=>'heure_depart','type'=>'time','label'=>'Heure de départ','class'=>'col','value' => $gestion->exists ? $gestion->heure_depart : ''])
                @include('shared.input', ['name'=>'date_depart','type'=>'date','label'=>'Date de départ','class'=>'col', 'value' => $gestion->exists ? $gestion->date_depart : ''])
